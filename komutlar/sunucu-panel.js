...
if(panel) return message.channel.send(`Bu sunucuda panel zaten ayarlanmış! Devredışı bırakmak için;  \`${prefix}sunucupanel sil\``)
      ...
      message.channel.send(new Discord.RichEmbed().setColor('RANDOM').setTitle('Sunucu Panel').setDescription('Gerekli Kanallar Kurulsun Mu?.').setFooter('Onaylıyorsan 15 saniye içerisinde "evet" yazmalısın.'))
.then(() => {
...
.then((collected) => { 
 ...
 message.guild.createChannel(`Aktif Üye • ${message.guild.members.filter(off => off.presence.status !== 'offline').size}`, 'voice').then(channel => channel.setParent(message.guild.channels.find(channel => channel.name === `${client.user.username} • Sunucu Panel`))).then(c => {
  ...
  message.channel.send(new Discord.RichEmbed().setColor('RANDOM').setTitle('Tamamlandı').setDescription('**Sunucu Panel Kurulumu Başarılı! <a:onay:740171695200927745>** \nPaneli silmek için \`!sunucu-panel sil\`  \n\n**Eğer Oda İsimlerini Değiştirirseniz Çalışmaz!**'))
